...
namespace Apps
{	
	PaperDraw::PaperDraw ():notFindCount(0),prevfind(false),color(CV_RGB(255,255,255)),phase(0) {
    ...
    switch (phase) {
      case 0: {
        ...
        if (ret) {
          ...
          dstPoints[1] = corners[CORNER_ROW-1];
          dstPoints[2] = corners[(CORNER_COLUMN-1)*CORNER_ROW];
          ...
          CvPoint d2 = this->convertCoordinates(cvPoint((int)(corners[CORNER_COUNT-1].x), (int)(corners[CORNER_COUNT-1].y)));
          printf("draw2 (%d, %d)\n", d2.x, d2.y);
          cvCircle(destImage, d2, 3, CV_RGB(255,0,0), -1);
           */

          CvPoint d3 = this->convertCoordinates(cvPoint(0,0));
          printf("draw3 (%d, %d)\n", d3.x, d3.y);
          cvCircle(destImage, d3, 10, CV_RGB(255,255,0), -1);

          CvPoint d4 = this->convertCoordinates(cvPoint(WIDTH, HEIGHT));
          printf("draw4 (%d, %d)\n", d4.x, d4.y);
          cvCircle(destImage, d4, 10, CV_RGB(0, 255,255), -1);
